...
export default function GanttView({
    ...
    const isEmpty = !renderData.some(
        (p) =>
            p.tasks &&
            p.tasks.some((t: any) => t.startDate && t.durationDay)
    );
    ...
    const totalDays = useMemo(() => {
        ...
        const min = Math.min(
            ...all.map((t) =>
                new Date(t.startDate || projectStartDate).getTime()
            )
        );
        ...
        <div className="bg-gray-900 p-4 rounded-xl border border-gray-700 overflow-x-auto">
            {isEmpty ? (
                ...
                <div className="min-w-[1200px] relative">
                    {/* TODAY LINE */}
                    <div
                        className="absolute top-0 bottom-0 w-[2px] bg-yellow-400 z-50"
                        style={{ left: `${todayLeft}px` }}
                    />

                    {/* HEADER */}
                    <div className="flex items-end mb-4 h-12 border-b border-gray-700">

                        {/* 🔥 ปุ่ม (อยู่ซ้ายสุดจริง ๆ) */}
                        <div style={{ width: sidebarWidth }} className="flex items-end px-2 pb-1">
                            <Button
                                color="secondary"
                                variant="flat"
                                radius="full"
                        ...
                        <div ref={headerRef} className="flex flex-1 items-end">
                            {Array.from({ length: totalDays }).map((_, i) => {
                                ...
                                if (d.getDate() !== 1) return <div key={i} className="flex-1" />;

                                return (
                                    <div key={i} className="flex-1 text-xs text-gray-300">
                    ...
                    {renderData.map((phase) => (
                        <div key={phase.id} className="mb-4">
                            <div
                                ...
                                className="flex items-center gap-2 bg-gray-800 p-2 rounded cursor-pointer"
                            >
                            ...
                            {expandedPhases[phase.id] !== false && (
                                <div className="relative bg-gray-800/40 p-2">
                                    ...
                                    >
                                        {Array.from({ length: Math.ceil(totalDays) }).map((_, i) => {
                                            ...
                                            return (
                                                <div
                                                    key={i}
                                                    className="flex-1 border-l border-gray-700/30"
                                                />
                                            );
                                        })}
                                    </div>
                                        ...
                                        return (
                                            <div key={i} className="flex items-center h-10">
                                                ...
                                                >
                                                    ...
                                                    <div className="text-xs text-gray-400 w-[50px] text-right">
                                                        {task.durationDay}d
                                                    </div>
                                                ...
                                                <div className="flex-1 relative h-6">
                                                    <div
                                                        className="absolute h-full bg-gray-700/50 rounded"
                                                        style={base}
                                                    />
