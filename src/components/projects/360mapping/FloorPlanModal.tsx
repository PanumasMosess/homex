...
export default function FloorPlanModal({
  ...
  handleMapClick,
  setSelectedMedia, // ส่งข้อมูล point ทั้งก้อนไปให้ Viewer แทน
  tempPoint,
  handleDeletePoint,
}: any) {
  return (
        ...
        {() => (
          <>
            <ModalHeader className="flex justify-between items-center pr-10">
              ...
              {isAddingMode ? (
                <Button color="danger" variant="flat" size="sm" onPress={handleCancelAddPoint} endContent={<CloseIcon size={16} />}>
                  ยกเลิกการวาง
                </Button>
              ) : (
                <Button color="primary" size="sm" onPress={() => setIsAddingMode(true)} endContent={<Plus size={16} />}>
                  เพิ่มจุด 360°
                </Button>
              )}
            </ModalHeader>
            ...
            <ModalBody className="p-4 bg-zinc-950">
              ...
              <div
                className={`relative w-full aspect-[4/3] md:aspect-[16/9] bg-black rounded-xl overflow-hidden group border border-white/10 ${
                  isAddingMode ? "cursor-crosshair" : "cursor-default"
                }`}
                onClick={handleMapClick}
              >
                <img src={selectedFloorPlan?.imageUrl} alt="Floor Plan" className={`w-full h-full object-cover transition-opacity duration-500 ${isAddingMode ? "opacity-30" : "opacity-60"}`} />
                ...
                {selectedFloorPlan?.points?.map((point: any) => {
                  return (
                    ...
                    >
                      {/* 🌟 แสดงชื่อจุดเมื่อเอาเมาส์ชี้ธรรมดา */}
                      <Tooltip content={point.title} placement="top" color="primary">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            // 🌟 ส่ง Point ไปทั้งก้อน ให้ Viewer จัดการต่อ
                      ...
                      <button
                        ...
                        className="absolute -top-2 -right-2 bg-danger text-white p-1 rounded-full opacity-0 group-hover/pin:opacity-100 transition-opacity z-20"
                      >
                ...
                {tempPoint && (
                  <div className="absolute flex flex-col items-center justify-center transform -translate-x-1/2 -translate-y-1/2 z-20" style={{ left: `${tempPoint.x}%`, top: `${tempPoint.y}%` }}>
